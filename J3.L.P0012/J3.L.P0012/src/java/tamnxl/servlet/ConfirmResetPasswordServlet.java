...
public class ConfirmResetPasswordServlet extends HttpServlet {
    ...
    static final Logger LOGGER = Logger.getLogger(ConfirmResetPasswordServlet.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String newPassowd = request.getParameter("key2");
        ...
        try {
            ...
            boolean rs = userInforDAO.confirmPassword(username, newPassowd);
            if (!rs) {
                request.setAttribute("invalidLink", "Đường link không hợp lệ!");
            } else {
               request.setAttribute("success", "Đổi mật khẩu thành công");
            }
            session.invalidate();
        } catch (NamingException ex) {
            LOGGER.error("ConfirmResetPasswordServlet - Naming " + ex.getMessage());
        } catch (SQLException ex) {
            LOGGER.error("ConfirmResetPasswordServlet - SQL " + ex.getMessage());
        } finally {
