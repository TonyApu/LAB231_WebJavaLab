...
public class BookingDAO implements Serializable {
    ...
    public ArrayList<BookingDTO> getListBooking(String username) 
            throws NamingException, SQLException {
        Connection con = null;
        PreparedStatement stm = null;
        ResultSet rs = null;
        ...
        try {
            con = DBHelper.makeConnection();
            if (con != null) {
                String sql = "SELECT bookID, username, bookDate, discountCode, discountValue, bookingStatus "
                        + "FROM Booking Where username = ? AND bookingStatus = ?";
                stm = con.prepareStatement(sql);
                stm.setString(1, username);
                stm.setString(2, "Confirmed");
                rs = stm.executeQuery();
                while (rs.next()) {
                    listBooking.add(new BookingDTO(rs.getString("bookID"), rs.getString("username"), rs.getString("bookDate"), rs.getString("discountCode"), rs.getFloat("discountValue"), rs.getString("bookingStatus")));
                }
            }
        } finally {
            if (rs != null) {
